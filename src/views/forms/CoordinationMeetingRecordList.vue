<template>
  <div class="coordination-list-page">
    ...
    <Card v-else class="coordination-list-card">
      <CardHeader class="d-flex align-items-center justify-content-between gap-3 flex-wrap">
        <div>
          <h5 class="mb-1">會議紀錄版本</h5>
          <p class="mb-0 small text-muted">每次施工協調會議建立一筆獨立紀錄與送審資料。</p>
        </div>
      ...
      <CardBody>
        ...
        <div v-else-if="!records.length" class="empty-state">
          <i class="fa fa-people-arrows fa-3x mb-3"></i>
          <div>目前沒有施工協調會紀錄</div>
          <div class="small mt-2">請新增本次會議紀錄。</div>
        </div>
        <div v-else class="row g-3">
          <div v-for="record in records" :key="record.id" class="col-12 col-xl-6">
            <article class="version-card h-100">
              <div class="d-flex gap-3">
                <div class="version-icon"><i class="fa fa-handshake"></i></div>
                <div class="flex-fill min-w-0">
                  <h5 class="text-truncate mb-2">{{ record.title }}</h5>
                  <div class="small text-muted mb-1">
                    會議日期：{{ record.meetingData.meetingDate || '尚未填寫' }}
                  </div>
                  <div class="small text-muted">
                    地點：{{ record.meetingData.location || '尚未填寫' }}
                  </div>
...
import {
  ...
  deleteCoordinationMeeting,
  listCoordinationMeetings,
  type CoordinationMeetingData,
  type CoordinationMeetingRecord,
...
const records = ref<CoordinationMeetingRecord[]>([])
const loading = ref(false)
const creating = ref(false)

function emptyData(): CoordinationMeetingData {
  const project = workspaceStore.currentProject
  const today = new Date().toISOString().slice(0, 10)
  return {
    projectName: project?.name ?? '',
    meetingTitle: '施工協調會議紀錄',
    meetingPeriod: '',
    meetingDate: today,
    meetingTime: '',
    location: '',
    ...
    recorder: '',
    hostAgency: '',
    contractorUnit: project?.contractorCompanyName || project?.contractorName || '',
    supervisionUnit: '',
    plannedPeriodProgress: '',
    plannedTotalProgress: '',
    actualTotalProgress: '',
    progressStatus: '',
    progressVariance: '',
    progressExplanation: '',
...
async function load() {
  ...
  loading.value = true
  try {
    records.value = await listCoordinationMeetings(constructionId.value)
  } catch (error) {
...
async function create() {
  ...
  creating.value = true
  try {
    const meetingData = emptyData()
    const record = await createCoordinationMeeting(constructionId.value, {
      title: `${pageTitle.value} ${meetingData.meetingDate}`,
      meetingData,
    })
    open(record.id)
  } catch (error) {
    console.error('[CoordinationMeeting] create failed', error)
    toastService.error('新增施工協調會紀錄失敗')
  } finally {
...
async function remove(record: CoordinationMeetingRecord) {
  if (!window.confirm(`確定刪除「${record.title}」及其送審紀錄？`)) return
  try {
    await deleteCoordinationMeeting(constructionId.value, record.id)
    await load()
  } catch (error) {
    console.error('[CoordinationMeeting] delete failed', error)
    toastService.error('刪除施工協調會紀錄失敗')
  }
}
...
.coordination-list-card :deep(.card-header),
.coordination-list-card :deep(.card-body) { background: transparent; }
.version-card {
  padding: 1.1rem;
...
.version-icon {
  ...
  border-radius: 0.65rem;
  color: #fff;
  background: linear-gradient(135deg, #9a5d16, #d49637);
}
